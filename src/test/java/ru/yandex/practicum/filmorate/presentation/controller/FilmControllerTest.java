...
public class FilmControllerTest {
        ...
        public void delete_unlikeFilm_success200() throws Exception {
                ...
                mvc.perform(delete(FilmController.CONTROLLER_ROUTE + FilmController.UNLIKE_FILM_SUBROUTE, filmId,
                                userId))
        ...
        public void delete_unlikeFilmNonExistinFilm_notFound404() throws Exception {
                ...
                Long userId = extractUserDto(addUser(VALID_USER_DTO_1.clone())).getId();
                mvc.perform(put(FilmController.CONTROLLER_ROUTE + FilmController.LIKE_FILM_SUBROUTE, filmId, userId));
                mvc.perform(delete(FilmController.CONTROLLER_ROUTE + FilmController.UNLIKE_FILM_SUBROUTE,
                                NON_EXISTING_ID, userId))
                                .andExpect(status().isNotFound());
        }
        ...
        public void put_likeFilm_nonExistingUser_notFound404() throws Exception {
                ...
                mvc.perform(put(FilmController.CONTROLLER_ROUTE + FilmController.LIKE_FILM_SUBROUTE, filmId,
                                NON_EXISTING_ID))
                                .andExpect(status().isNotFound());
        }
        ...
        public void put_likeFilm_nonExistingFilm_notFound404() throws Exception {
                ...
                Long userId = extractUserDto(addUser(VALID_USER_DTO_1.clone())).getId();
                mvc.perform(put(FilmController.CONTROLLER_ROUTE + FilmController.LIKE_FILM_SUBROUTE, NON_EXISTING_ID,
                                userId))
                                .andExpect(status().isNotFound());
        }
        ...
        public void get_films_getDirectorFilms_success200WithCollectionOfDtos() throws Exception {
                ...
                mvc.perform(get(FilmController.CONTROLLER_ROUTE + FilmController.GET_DIRECTOR_FILMS_SUBROUTE,
                                VALID_DIRECTOR_DTO_1.getId()))
                                .andExpect(status().isOk())
                                .andExpect(content()
                                                .contentTypeCompatibleWith(MediaType.APPLICATION_JSON));
        }
        ...
        private FilmDto extractFilmDto(ResultActions actions)
        ...
        private UserDto extractUserDto(ResultActions actions)
                        throws UnsupportedEncodingException, JsonProcessingException {
        ...
        @Test
        @DisplayName("Get common films for existing users")
        public void get_commonFilms_existingUsers_success200() throws Exception {
                Long filmId1 = extractFilmDto(addFilm(VALID_FILM_DTO_1.clone())).getId();
                Long filmId2 = extractFilmDto(addFilm(VALID_FILM_DTO_2.clone())).getId();

                Long userId1 = extractUserDto(addUser(VALID_USER_DTO_1.clone())).getId();
                Long userId2 = extractUserDto(addUser(VALID_USER_DTO_2.clone())).getId();
                Long userId3 = extractUserDto(addUser(VALID_USER_DTO_3.clone())).getId();
                ...
                mvc.perform(put(FilmController.CONTROLLER_ROUTE + FilmController.LIKE_FILM_SUBROUTE, filmId2, userId1));
                mvc.perform(put(FilmController.CONTROLLER_ROUTE + FilmController.LIKE_FILM_SUBROUTE, filmId2, userId2));
                mvc.perform(put(FilmController.CONTROLLER_ROUTE + FilmController.LIKE_FILM_SUBROUTE, filmId2, userId3));

                mvc.perform(get(FilmController.CONTROLLER_ROUTE + FilmController.GET_COMMON_FILMS_SUBROUTE)
                                .param("userId", String.valueOf(userId1))
                                .param("friendId", String.valueOf(userId2)))
                                .andExpect(status().isOk())
                                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
        ...
        @Test
        @DisplayName("Get common films with non-existing userId")
        public void get_commonFilms_nonExistingUser_notFound404() throws Exception {
                Long userId = extractUserDto(addUser(VALID_USER_DTO_1.clone())).getId();

                mvc.perform(get(FilmController.CONTROLLER_ROUTE + FilmController.GET_COMMON_FILMS_SUBROUTE)
                                .param("userId", String.valueOf(NON_EXISTING_ID))
                                .param("friendId", String.valueOf(userId)))
                                .andExpect(status().isNotFound());
        }

        @Test
        @DisplayName("Get common films with non-existing friendId")
        public void get_commonFilms_nonExistingFriend_notFound404() throws Exception {
                Long userId = extractUserDto(addUser(VALID_USER_DTO_1.clone())).getId();

                mvc.perform(get(FilmController.CONTROLLER_ROUTE + FilmController.GET_COMMON_FILMS_SUBROUTE)
                                .param("userId", String.valueOf(userId))
                                .param("friendId", String.valueOf(NON_EXISTING_ID)))
                                .andExpect(status().isNotFound());
        }
}
